  ...
  def patients_with_start_reason(reasons)
    ...
    if reasons == ['who_stage_1_or_2_cd4']
      reasons = ['CD4 Count < 250','CD4 percentage < 25',
                 'CD4 Count < 350','CD4 Count < 500','CD4 Count < 750']
...
=end
  ...
  def patients_by_regimen
    ...
    sql_str=<<EOF
SELECT r.patient_id patient_id , r.category regimen_category FROM `patient_registration_dates` 
INNER JOIN patient ON patient.patient_id = patient_registration_dates.patient_id  
INNER JOIN patient_historical_outcomes ON patient_historical_outcomes.patient_id = patient.patient_id AND patient_historical_outcomes.`outcome_concept_id` = #{on_art_concept_id}  
INNER JOIN patient_historical_regimens r ON r.patient_id = patient.patient_id  
AND r.dispensed_date = (SELECT MAX(dispensed_date) FROM patient_historical_regimens 
...
EOF
    ...
    PatientHistoricalRegimen.find_by_sql(sql_str).map do |r|
      category = r.regimen_category.blank? ? "Unknown" : r.regimen_category 
      regimen_hash[category] = [] if regimen_hash[category].blank?
      regimen_hash[category] << Patient.find(r.patient_id) 
    end
